...
<NFe xmlns="http://www.portalfiscal.inf.br/nfe">`,
  ...
  it('should not generate a valid SHA256 signature :(', async () => {
    if (certificate === undefined) return;
...
<NFe xmlns="http://www.portalfiscal.inf.br/nfe">
  <infNFe Id="NFe42250903916076000583550560000005651509037627" versao="4.00">
    ...
    expectIsLeft(signed);
    expect(signed.value).toStrictEqual(
      new NFeTsError(
        `CNG signing failed. The certificate's key provider may not fully support CNG signing operations, or the key may require user interaction (e.g., smart card PIN). Error: -2146893785`,
      ),
    );

    // TODO: wincrypt signature with algo SHA256 is not working, use only SHA1 instead
