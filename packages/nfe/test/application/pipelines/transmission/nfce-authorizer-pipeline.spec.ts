...
${NFe}`
    ...
    const NFeOrLeft = await builder.assemble();
...
describe('nfe with failures authorizer pipeline (unit)', () => {
                  ...
                  protNFe: [
                    ...
                    },
                    {
                      ...
                      infProt: {
                        ...
                        digVal: '123456789012345',
                        cStat: '742',
                        xMotivo: 'NFC-e com grupo do IPI',
                      },
                    },
                  ],
                },
  ...
  it('should attempt to authorize a batch of nfe and return the correct response with protocol to first and not to second, due the rejection', async () => {
    ...
    const firstXmlOrLeft = await firstBuilder.assemble();

    expectIsRight(firstXmlOrLeft);
    expect(firstXmlOrLeft.value).toBeDefined();

    const firstNFe = `<NFe xmlns="http://www.portalfiscal.inf.br/nfe">
  <infNFe Id="NFe52240646755763000143550990000080181785272515" versao="4.00">
    <ide>
      <cUF>52</cUF>
      <cNF>78527251</cNF>
  ...
  <Signature xmlns="http://www.w3.org/2000/09/xmldsig#">
    ...
    expect(firstXmlOrLeft.value).toBeDefined();
    expect(firstXmlOrLeft.value).toStrictEqual(
      `<?xml version="1.0" encoding="UTF-8"?>
${firstNFe}`
        .replace(/\s+/g, ' ')
        .replace(/(>)\s+(<)/g, '$1$2')
    ...
    const firstNFeOrLeft = await firstBuilder.assemble();
    expectIsRight(firstNFeOrLeft);

    const secondBuilder = xmlBuilderPipeline.build((builder) =>
      builder
        .infNFe({ versao: '4.00' })
        .ide({
          cUF: '52',
    ...
    const secondXmlOrLeft = await secondBuilder.assemble();

    expectIsRight(secondXmlOrLeft);
    expect(secondXmlOrLeft.value).toBeDefined();

    const secondNFe = `<NFe xmlns="http://www.portalfiscal.inf.br/nfe">
    <infNFe Id="NFe52240646755763000143550990000080181785272515" versao="4.00">
    <ide>
      <cUF>52</cUF>
      <cNF>78527251</cNF>
    ...
    expect(secondXmlOrLeft.value).toBeDefined();
    expect(secondXmlOrLeft.value).toStrictEqual(
      `<?xml version="1.0" encoding="UTF-8"?>
    ${secondNFe}`
        .replace(/\s+/g, ' ')
        .replace(/(>)\s+(<)/g, '$1$2')
    ...
    const secondNFeOrLeft = await secondBuilder.assemble();
    ...
    expect(response.retEnviNFe.protNFe).toStrictEqual([
      ...
      },
      {
        ...
        infProt: {
          ...
          verAplic: '1.0',
          chNFe: '123456789012345',
          dhRecbto: new Date('2025-12-22T18:40:33.060Z').toISOString(),
          nProt: '123456789012345',
          digVal: '123456789012345',
          cStat: '742',
          xMotivo: 'NFC-e com grupo do IPI',
    ...
    expect(xml[0]).toBeDefined();
    expect(xml[0]).toStrictEqual(
      `<?xml version="1.0" encoding="UTF-8"?>
<nfeProc xmlns="http://www.portalfiscal.inf.br/nfe" versao="4.00">
${firstNFe}
<protNFe versao="4.00">
  <infProt>
    ...
    expect(xml[1]).toBeDefined();
    expect(xml[1]).not.toContain('<protNFe>');
    expect(xml[1]).toStrictEqual(
      `<?xml version="1.0" encoding="UTF-8"?>
${secondNFe}`
        .replace(/\s+/g, ' ')
        .replace(/(>)\s+(<)/g, '$1$2')
