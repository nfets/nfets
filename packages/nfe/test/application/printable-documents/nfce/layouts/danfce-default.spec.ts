...
import { DanfceDefaultPdfDocument } from '@nfets/nfe/application/printable-documents/nfce/layouts/danfce-default';
...
describe('DanfceDefault', () => {
  let file: string;
  let stream: Writable;
  let danfce: DanfceDefaultPdfDocument;

  beforeEach(() => {
    const tmp = tmpdir();
    file = path.join(tmp, `${new Date().getTime()}.pdf`);
    stream = createWriteStream(file);
    danfce = new DanfceDefaultPdfDocument(stream);
  });
  ...
  it('should build a default danfce layout with a complete xml production protocoled', async () => {
    ...
    try {
      ...
      expectInOrder(text)
        ...
        // Sample items from XML
        .toContain('Coca cola Lata 350ml')
        .toContain('319238921')
        .toContain('319238921312312312312')
        .toContain('asdijas98djas98')
        .toContain(
